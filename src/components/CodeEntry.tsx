...
const LanguageCopy = [
  {
    helpInfo:
      "Please enter the code attached on the bin and your unique code (ABN). The weight of waste will be captured by the sensor and linked to your account automatically",
  ...
  {
    helpInfo:
      "请输入附在垃圾箱上的识别码以及您的ABN。垃圾桶上的检测芯片会自动记录您所扔垃圾的重量，并关联到您的账号",
      ...
      <CodeEntryContainer>
        <EntryFormContainer>
          ...
          <SubmitButton onClick={onSubmitClicked}>
            {LanguageCopy[language].submit}
          </SubmitButton>
          {showUpdatedMsg && (
            <UpdateMsg>{LanguageCopy[language].updateMessage}</UpdateMsg>
          )}
        </EntryFormContainer>
        <Footer>
          <Row>
            <span style={{ paddingBottom: "4px" }}>IN COLLABORATION WITH</span>
